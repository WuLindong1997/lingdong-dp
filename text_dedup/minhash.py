...
import sys
    ##修改下面的参数
sys.path.append("/mnt/vepfs/lingxin/Pretrain-data/leileqi_test/dedup_code/text-dedup")
...
if __name__ == "__main__":  # pragma: no cover
    ...
    ##修改下面的参数
    ##数据集目录
    args.path = '/mnt/vepfs/lingxin/Pretrain-data/leileqi_test/xiaohongshu_dedup_test/xiaohongshu_save_emoji_end'
    ##内容的key
    args.column = 'text'
    ##输出目录
    args.output = '/mnt/vepfs/lingxin/Pretrain-data/leileqi_test/xiaohongshu_dedup_test/xiaohongshu_deduped'
    ##缓存目录
    args.cache_dir = '/mnt/vepfs/lingxin/Pretrain-data/leileqi_test/xiaohongshu_dedup_test/xiaohongshu_cache'
    ##进程数
    args.num_proc = 80
    ##batch数d
    ...
    data_files = os.listdir(args.path)
    data_files = [os.path.join(args.path, data_file) for data_file in data_files][:]
    ...
    # args.R = 1
    args.threshold = 0.8
    args.false_positive_weight = 0.5
    args.false_negative_weight = 0.5
    ######################################
    ...
    with timer("Total"):
        ...
        # # files = []
        # # for i in range(B):
        # #     path = f'hash_table_{i}.db'
        # #     files.append(path)
        # #     hash_tables.append(shelve.open(path))

        # hash_file = shelve.open('hash_file.db')
        # hash_file['tables'] = HASH_TABLES
        ...
        #                 # 将数据存储在shelve对象中
        #                 # if H not in hash_file['tables'][i]:
        #                 #     hash_file['tables'][i][H] = set()
        #                 hash_file['tables'][i][H].add(key)
...
###########################################
    ...
    #         # Define a function to process a batch of embedded data
    #         def process_batch(batch, hash_tables):
    #             for key, Hs in zip(batch["__id__"], batch["__signatures__"]):
    #                 for i, H in enumerate(Hs):
    #                     hash_tables[i][H].add(key)
    ...
    #         # NUM_TABLES = 10
    #         # HASH_TABLES = [defaultdict(set) for _ in range(NUM_TABLES)]

    #         # Define the batch processing function for multiprocessing
    #         def batch_processing(queue, hash_tables,queue2):
    #             while True:
    #                 batch = queue.get()
    #                 if batch is None:
                        
    #                     queue2.put(hash_tables)
    #                     break
                        
    #                 process_batch(batch, hash_tables)
    #         # Define a function to extract hash tables from processes 
    #         def extract_hash_tables(processes):
    #             hash_tables = []
    #             for p in processes:
    #                 hash_tables.append(p.join())  # Join process and get return value
    #             return hash_tables


    # # ... put batches on queue ...

        
    #         # Create a multiprocessing queue and processes
    #         queue = mp.Queue()
    #         queue2 = mp.Queue()
    #         processes = [mp.Process(target=batch_processing, args=(queue, [defaultdict(set) for _ in range(B)],queue2)) for _ in range(args.num_proc)]

    #         # Start the multiprocessing processes
    #         for p in processes:
    #             p.start()

    #         # Put the batches into the multiprocessing queue
    #         for i in tqdm(range(0, len(embedded), args.batch_size), dynamic_ncols=True, desc="Iterating MinHashes..."): 

    #             queue.put(embedded[i:i+args.batch_size])

    #         # Signal the multiprocessing processes to exit
    #         for _ in range(args.num_proc):
    #             queue.put(None)

    #         # # Wait for the multiprocessing processes to finish
    #         # for p in processes:
    #         #     p.join()

            
    #         # # Wait for processes to finish and extract hash tables
    #         # hash_tables = extract_hash_tables(processes)
    ...
    #         # Merge the hash tables
    #         for i in range(B):
    #             for _ in tqdm(range(args.num_proc), desc='Merging sub processes..'):
    #                 p_hash_tables = queue2.get()
    #                 for ht in p_hash_tables:
    #                     HASH_TABLES[i].update(ht[i])
    #         # for i in range(B):

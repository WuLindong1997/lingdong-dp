...
if __name__ == "__main__":  # pragma: no cover
    ...
    ##缓存目录
    args.cache_dir = '/mnt/lingxin/Pretrain-data/wulindong/weixin_cache3'
    ...
    for root,dir,filenames in os.walk(args.path):
        for filename in filenames:
            if filename.endswith('.json') and 'weixin_page.2019' in  filename:
    ...
    while start_index < data_files_len:
        
        batch_files = data_files[start_index:start_index+batch_size]
        # 在这里处理batch_files列表中的文件
        # ...
        start_index += batch_size
        
        save_name = f'weixin_page.2019_{start_index}.jsonl'
        ...
        with timer("Total"):
            with timer("Loading"):
                if args.local:
                    ...
                    ds = load_dataset("json", data_files=batch_files\
            ...
            with timer("Saving"):
                ...
                path = os.path.join(args.output,save_name)
                final_data.to_json(path, orient="records", lines=True, force_ascii=False)
                path = os.path.join(args.output_duped, save_name)

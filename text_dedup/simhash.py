...
if __name__ == "__main__":
    ...
    args.output_abandoned = '/mnt/vepfs/lingxin/Pretrain-data/leileqi_test/xiaohongshu_dedup_test/test_deduped_abandoned'
    ...
    data_files = [os.path.join(args.path, data_file) for data_file in data_files][:1]
    # data_files = ['/mnt/vepfs/lingxin/pretrain/leileqi_test/test_dedup.jsonl']
    args.parallel = False
    ######################################
    ...
    with timer("Total"):
        ...
        import numpy as np
        SyncManager.register('UnionFind', UnionFind)
        SyncManager.register('defaultdict', defaultdict)
        manager = SyncManager()
        manager.start()
        shared = manager.Namespace()
        shared.uf = manager.UnionFind()
        shared.bk = manager.defaultdict(list)
    
        # shared_uf = manager.Proxy(UnionFind())
        # shared_bk = manager.Proxy(defaultdict(list))
        # # shared_uf = Manager.UnionFind()
        # shared_ns = manager.Namespace()
        # shared_ns.BUCKETS = BUCKETS
        # shared_ns.uf = uf
        ...
        # TODO Create multiple BUCKETS for parallelization
            ...
            lock = Lock()
            lock2 = Lock()
            start_indices = np.array_split(range(len(embedded)), args.num_proc)
            def worker(shared, embedded):
                # global lock
                # global lock2
                # global BUCKETS
                # global uf
                ...
                with timer("Clustering"):
                    for i in tqdm(
                        ...
                        for idx, keys, sig in tqdm(
                            ...
                            for key in keys:
                                key = tuple(key)
                                iters = shared.bk.get(key)
                                if iters is not None:
                                    for idy, other_fingerprint in iters:
                                        if idy in neighbors:
                                            continue
                                        if _hamming_distance(sig, other_fingerprint) <= args.bit_diff:
                                            neighbors.add(idy)
                                    shared.bk.get(key).append((idx, sig))
                            for idy in neighbors:
                                shared.uf.union(idx, idy)
                lock.release()
            processes = [Process(target=worker, args=(shared, embedded.select(range(start_indices[i][0], start_indices[i][-1]+1)),)) for i in range(args.num_proc)]
            for p in processes:
                p.start()
            for p in processes:
                p.join()
            ...
            BUCKETS = shared.bk
            uf = shared.uf._getvalue()
        import pickle
        path = 'unionfind_paral.pkl' if args.parallel else 'unionfind_seque.pkl'
        ...
        with timer("Filtering"):
            ...
            #下面存储一下滤掉的数据
            final_data_filtered = ds.filter(
                function=lambda record, idx: record["__cluster__"] != idx,
                with_indices=True,
                num_proc=os.cpu_count() if args.num_proc==None else args.num_proc,
                desc="Filtering clusters filtered...",
            )

        with timer("Saving"):
            final_data = final_data.remove_columns(["__cluster__"])
            final_data.save_to_disk(args.output)
            final_data_filtered.save_to_disk(args.output_abandoned)
            if args.debug:

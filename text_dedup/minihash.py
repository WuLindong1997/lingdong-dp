...
# import sys
#     ##修改下面的参数
sys.path.append("/mnt/vepfs/lingxin/Pretrain-data/wulindong")
...
def embed_func(
    ...
    *,
    ...
    content = extract_chinese(content)
    ...
    tokens = ngrams(content, ngram_size)
    tokens = map(lambda x: ''.join(x), tokens)
    [m.update(x.encode('utf8')) for x in tokens]
    Hs = m.digest().reshape(-1,hashranges)
    Hs = [''.join(map(str, row)) for row in Hs]
    # print(f"length of Hs is {len(Hs)}")
...
if __name__ == "__main__":  # pragma: no cover
    ...
    ##修改下面的参数
    ##数据集目录
    args.path = '/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_end'
    ##内容的key
    args.column = 'text'
    ##输出目录
    args.output = '/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_dudmp'
    ##判定为重复文档的保存目录
    args.output_duped = '/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_dudmp_abandon'
    ##缓存目录
    args.cache_dir = '/mnt/vepfs/lingxin/Pretrain-data/wulindong/weixin_cache'
    ##进程数
    args.num_proc = 32
    ##batch数d
    ...
    for root,dir,filenames in os.walk(args.path):
        for filename in filenames:
            if filename.endswith('.json') and filename in 'weixin_page.2015':
                data_files.append(os.path.join(root,filename))
    # data_files = os.listdir(args.path)
    # if os.path.isdir(os.path.join(args.path, data_files[0])):
    #     data_files = [os.path.join(args.path, i, j) for i in data_files for j in os.listdir(os.path.join(args.path, i))]
    # else:
    ...
    # args.R = 1
    args.threshold = 0.7
    args.false_positive_weight = 0.7
    ...
    HASH_RANGES = R
    HASH_TABLES: List[Dict[str, Set]] = [defaultdict(set) for _ in range(B)]

    steps = 20
    idx = 0
    while 1:
        if idx*steps >= len(data_files):
            break
        elif (idx+1)*steps >= len(data_files):
            data_files_slice = data_files[idx*steps:]
        else:
            data_files_slice = data_files[idx*steps:(idx+1)*steps]
        with timer("Total"):
            with timer("Loading"):
                if args.local:
            ...
            with timer("Saving"):
                ...
                # final_data_duped.save_to_disk(args.output_duped)
                name = 'weixin2021_{}.jsonl'.format(idx)
                path = os.path.join(args.output,name)
                final_data.to_json(path, orient="records", lines=True, force_ascii=False)
                path = os.path.join(args.output_duped, name)
                final_data_duped.to_json(path, orient='records', lines = True, force_ascii=False)
                if args.debug:
        ...
        os.system('rm -rf {}/*'.format(args.cache_dir))
        idx = idx + 1

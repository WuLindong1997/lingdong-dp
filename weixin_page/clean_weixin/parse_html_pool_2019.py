...
def get_args():
    ...
    # dictionary or file
    args_parser.add_argument('--dataset_path', type=str,default='/mnt/vepfs/lingxin/pretrain/wulindong/weixin_page.2019-10-01')
    ...
    # 读取文件内容并进行处理
        with multiprocessing.Pool(args.pool_num) as pool:
            ...
            while True:
                ...
                if not html_txt:
                    flag = 0
                # pages = re.findall(r"<!DOCTYPE html>(.*?)</html>", html_txt, re.DOTALL)
                pages = re.findall(r"<html>.*?</html>", html_txt, re.DOTALL)
                batch_list = pages
                print(f"Read {len(batch_list)} lines of data")
                results = list(tqdm(pool.imap(process_data, batch_list),total=len(batch_list)))

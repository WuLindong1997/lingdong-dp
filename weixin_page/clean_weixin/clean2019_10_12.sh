...
for file in /pretrain-data-bucket/pretrain_other/pretrain_else/weixin/page/weixin_page.2022-{09,10,11,12}*.7z; do
    # 使用basename命令获取文件名部分
    filename_d=$(basename $file .7z)
    if [ -d "/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_clean/$filename_d" ];
    then
        echo "/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_clean/$filename_d exists. skip."
    else
        7z x -o/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_un7z2019_12 $file -mmt20
        echo '7z解压！！！！'
        mkdir /pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_clean/$filename_d
        
        python /mnt/lingxin/vepfs/pretrain/wulindong/weixin_page/code/clean_weixin/parse_html_pool_2022.py \
            --dataset_path   /pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_un7z2019_12/$filename_d \
            --save_path /pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_clean/$filename_d \
            --pool_num  16
        wait 
        rm -rf /pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_un7z2019_12/*

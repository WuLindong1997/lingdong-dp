...
# @staticmethod
def save_filtered_file(document):
    with open("/mnt/vepfs/lingxin/Pretrain-data/wulindong/bookdata/logs/filtered.json","a",encoding='utf-8') as f:
        json.dump({'text':document},f,ensure_ascii=False)
        f.write('\n')
...
class DatasetFiltering:
    ...
    def filtering(self,sava_path):
        ...
        result = []
        for line in tqdm(self.ds["train"]):
            judge = func_dataset_filtering(line)
            # if len(text['text'])<2:
            #     text['save'] = "False"
            judge_dirc = {"text":line['text'],'save':judge}
            result.append(json.dumps(judge_dirc,ensure_ascii=False))
        with open(sava_path,'w',encoding='utf-8')as file1:
            file1.write('\n'.join(result))

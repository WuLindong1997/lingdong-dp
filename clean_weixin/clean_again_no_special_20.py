...
def process_json(data,toxic_words):
    ...
    chinese_chars = float(len(''.join(re.findall(r'[\u4e00-\u9fff]+', text))))
    english_words = float(len(re.findall(r'\b[A-Za-z]+\b', text)))
...
def get_args():
    ...
    # dictionary or file
    args_parser.add_argument('--dataset_path', type=str,default='/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_dudmp')
    ...
    args_parser.add_argument('--save_path', type=str,default='/pretrain-data-bucket/pretrain_other/pretrain_else/weixin_page_dudmp_again')
